...
public class DbgConnection {
    ...
    public List<RoutineInfo> fetchRoutines(String schema) throws DbgException {
        String sql =
            ...
            "  AND LEFT(ROUTINE_NAME, 5) <> '_dbg_' " +
            "  AND LEFT(ROUTINE_NAME, 6) <> '_orig_' " +
    ...
    public RoutineMetadata fetchRoutineMetadata(String schema, String name, String type)
            throws DbgException {
    ...
    public String findRoutineType(String schema, String name) throws DbgException {
        try (PreparedStatement ps = conn.prepareStatement(
    ...
    public void saveOriginal(String name, String type, String ddl, String sessionId)
        ...
        String sql =
            ...
            "routine_type=?, original_ddl=?, session_id=?";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, name); ps.setString(2, type);
            ps.setString(3, ddl);  ps.setString(4, sessionId);
            ps.setString(5, type); ps.setString(6, ddl); ps.setString(7, sessionId);
            ps.executeUpdate();
        } catch (SQLException e) {
